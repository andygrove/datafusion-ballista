...
/// over for the lifetime of the stream, so the connection stays checked out of the
/// pool until the response body is fully read — it cannot be reused, evicted, or
/// closed mid-stream. On a transport error the connection is discarded rather than
/// returned, since it is no longer trustworthy.
struct PooledPartitionStream {
    inner: SendableRecordBatchStream,
    /// `Some` until the stream errors. Dropping it returns the connection to the
    /// pool; `discard()` on error prevents reuse of a broken connection.
...
impl Stream for PooledPartitionStream {
    ...
    fn poll_next(
        ...
        // state; discard it so it is not returned to the pool.
...
async fn fetch_partition_remote(
    ...
    if let Some(pool) = client_pool {
        ...
        match pooled
            ...
            .await
        {
            // Hold the pooled connection checked out for the lifetime of the
            // response stream. Returning it to the pool here (before the body is
            // read) would let another fetch reuse the same connection while this
            // one is still streaming — i.e. multiplex over it — which deadlocks on
            // the h2 flow-control window under heavy shuffle load. The connection
            // is returned (or discarded on a transport error) only once the stream
            // finishes.
